...
The frontend is built with **Angular** using the **standalone** component API instead of NgModules. Each component explicitly declares its own dependencies through the `imports` property, resulting in a simpler and more modular project structure.

### Application Structure
- `App` is the root component of the application.
- The root template contains only a `router-outlet`, which serves as the entry point for all routed pages.
  - `app-popup` — renders application-wide popup dialogs.
  - `app-powerlifting-chat` — provides the integrated AI assistant.
