...
- Triggers the initial database synchronization through [SyncService](sync-service.md#syncservice).
...
3. Sends the role update through [DeviceRoleService](connection_service.md#deviceroleservice).
...
- ### check()
Requests the current connection state from [ConnectionsService](connection_service.md#connectionsservice).
...
- #### backToRole()
  ...
  - Updates the device role to `null` through [DeviceRoleService](connection_service.md#deviceroleservice).
